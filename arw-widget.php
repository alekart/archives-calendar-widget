...
function archives_month_view( $args, $sql ) {
	...
	$templates = array(
		"pages-container" => '<div class="archives-years">',
		"year-container"  => '<div class="year %s" rel="%s" data-year="%s" data-month="%s">%s</div>',
		"week-row"        => '<div class="week-row %s">%s</div>',
		"day"             => '<span class="day %s">%s</span>',
		"day-link"        => '<a href="%s">%s</a>'
	);

	// for each month
	for ( $i = 0; $i < $totalmonths; $i ++ ) {

		$dayswithposts = getDaysWithPosts( $months[ $i ]->year, $months[ $i ]->month, $post_type, $categories, $cats );
		...
		if ( $i == $totalmonths - 1 ) {
			array_push( $yearClasses, 'last' );
		}
		if ( $archiveYear == $months[ $i ]->year && $archiveMonth == $months[ $i ]->month ) {
			array_push( $yearClasses, 'current' );
		}

		$month   = build_month_matrix( $months[ $i ]->year, $months[ $i ]->month );
		$week    = '';
		$allGrid = '';
		...
		for ( $day = 0; $day < count( $month ); $day ++ ) {
			...
			if ( $date === 0 ) {
				$classes = array( 'noday' );
				$week .= sprintf( $templates['day'], join( ' ', $classes ), '' );
			} else {
				$classes = array();

				if ( $date == $todayDay && $months[ $i ]->year == $todayYear && $months[ $i ]->month == $todayMonth ) {
					array_push( $classes, 'today' );
				}

				if ( in_array( $date, $dayswithposts ) ) {
					array_push( $classes, 'has-posts' );
					$href = get_day_link( $months[ $i ]->year, $months[ $i ]->month, $date ) . make_arcw_link( $post_type, $cats );

					$dayContent = sprintf( $templates['day-link'], $href, $date );
				} else {
					$dayContent = $date;
				}

				$week .= sprintf( $templates['day'], join( ' ', $classes ), $dayContent );
			}

			if ( ( $day + 1 ) % 7 == 0 ) {
				$classes = array();
				$allGrid .= sprintf( $templates['week-row'], join( ' ', $classes ), $week );
				$week = '';
			}
		}

		$cal .= sprintf( $templates['year-container'], join( ' ', $yearClasses ), $i, $months[ $i ]->year, $months[ $i ]->month, $allGrid );
	}

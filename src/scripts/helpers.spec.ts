  ...
  describe('getNumberOfDaysForMonth', () => {
    it('should return correct number of days for non February month without year', () => {
      ...
      for (let i = 1; i <= 12; i += 1) {
        if (i !== 2) {
          expect(Helpers.getNumberOfDaysInMonth(i)).toBe(months[i - 1]);
        }
      }
    });

    it('should return 29 days for February in leap year', () => {
      expect(Helpers.getNumberOfDaysInMonth(2, 2020)).toBe(29);
      expect(Helpers.getNumberOfDaysInMonth(2, 2016)).toBe(29);
    });

    it('should return 28 days for February in non-leap year', () => {
      expect(Helpers.getNumberOfDaysInMonth(2, 2019)).toBe(28);
      expect(Helpers.getNumberOfDaysInMonth(2, 2021)).toBe(28);
    });

    it('should throw if year is not provided for February', () => {
      expect(() => Helpers.getNumberOfDaysInMonth(2)).toThrow();
    });
  });
});
